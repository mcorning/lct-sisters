...
export default {
  ...
  methods: {
    ...
    */
    ...
    getMarker(marker) {
      ...
      this.marker = marker;
      // TODO Refactor for State
      this.place = this.state.find(marker.place_id);
    ...
    updateName(name) {
      // TODO Refactor for State
      this.state
        .updateFieldPromise(this.place.place_id, { name: name })
        .then((p) => {
    ...
    openInfoWindowWithSelectedPlace(place) {
      try {
        ...
        // TODO Refactor for State
        const { lat, lng } = this.state.getPosition(this.place.place_id);
    ...
    addVisit(nativeEvent, startTime = Date.now(), endTime, shift) {
      ...
      const selectedSpace = {
        ...
        // TODO Refactor for State
        plus_code:
          this.state.getPosition(this.place.place_id).plus_code || 'NA',
        startTime: startTime,
        endtime: endTime,
        shift: shift,
      };
  ...
  watch: {
    ready() {
      if (this.ready) {
        console.log('GoogleMap Ready. state:', this.state);
  ...
  mounted() {
    ...
    Promise.all([Place.$fetch(), Visit.$fetch(), Setting.$fetch()])
      .then((entities) => {
        ...
        const settings = entities[2].settings[0] || [];

        self.updateState({ places, visits, settings });
